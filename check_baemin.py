...
def fetch_api(url):
    """curl_cffi를 사용하여 실제 Chrome 브라우저의 TLS 보안 패킷으로 완벽 위장 타격 (Cloudflare 403 우회)"""
    headers = {
    ...
    resp = async_requests.get(url, headers=headers, impersonate="chrome120", timeout=20)
    return resp.text
...
def collect_all_data():
    ...
    while True:
        ...
        if isinstance(data, dict) and data.get("status") in (401, 403):
            raise PermissionError("쿠키 세션이 만료되었거나 접속이 차단되었습니다.")
...
if __name__ == "__main__":
    log.info("🚀 [Cloudflare 우회 수집기] 데이터 수집 및 전송 시작")
    try:
